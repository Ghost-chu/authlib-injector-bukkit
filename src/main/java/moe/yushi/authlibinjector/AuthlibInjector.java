...
public final class AuthlibInjector {
	...
	public static final String[] nonTransformablePackages = new String[] { "java.", "javax.", "com.sun.", "com.oracle.",
			"jdk.", "sun.", "org.apache.", "com.google.", "oracle.", "com.oracle.", "com.paulscode.", "io.netty.",
			"org.lwjgl.", "net.java.", "org.w3c.", "javassist.", "org.xml.", "org.jcp.", "paulscode.", "com.ibm.",
			"joptsimple.", "moe.yushi.authlibinjector.", "org.graalvm.", "org.GNOME.", "it.unimi.dsi.fastutil.",
			"oshi.", "com.jcraft.jogg.", "com.jcraft.jorbis.", "org.objectweb.asm.", "org.yaml.snakeyaml.",
			"gnu.trove.", "jline.", "org.json." };

	// ==== System Properties ===

	/**
	 * Stores the API root, should be set before {@link #bootstrap(Consumer)} is
	 * invoked.
	 */
	...
	/**
	 * The side that authlib-injector runs on. Possible values: client, server.
	 */
	...
	public static void bootstrap(Consumer<ClassFileTransformer> transformerRegistry)
			throws InjectorInitializationException {
	...
	private static Optional<String> getPrefetchedResponse() {
		...
		if (prefetched == null) {
			...
			if (prefetched != null) {
				Logging.LAUNCH.warning(PROP_PREFETCHED_DATA_OLD + " option is deprecated, please use "
						+ PROP_PREFETCHED_DATA + " instead");
			}
		}
	...
	private static Optional<YggdrasilConfiguration> configure() {
		...
		if (!prefetched.isPresent()) {
				...
				for (;;) {
					...
					if (ali.isPresent()) {
						...
						if (redirectCount >= REDIRECT_LIMIT) {
							Logging.CONFIG.severe("Exceeded maximum number of redirects (" + REDIRECT_LIMIT
									+ "), refusing to redirect to: " + apiRoot);
							throw new InjectorInitializationException();
						}
						redirectCount++;
			...
			try {
				...
				Logging.CONFIG
						.severe("Unable to decode metadata: " + e + "\n" + "Encoded metadata:\n" + prefetched.get());
				throw new InjectorInitializationException(e);
			}
		}
		...
		try {
			...
			Logging.CONFIG
					.severe("Unable to parse metadata: " + e.getCause() + "\n" + "Raw metadata:\n" + metadataResponse);
			throw new InjectorInitializationException(e);
		}
	...
	private static Optional<String> getApiLocationIndication(URLConnection conn) {
		return Optional.ofNullable(conn.getHeaderFields().get("X-Authlib-Injector-API-Location"))
				.flatMap(list -> list.isEmpty() ? Optional.empty() : Optional.of(list.get(0))).flatMap(indication -> {
					...
					try {
						...
						Logging.CONFIG.warning(
								"Failed to resolve absolute ALI, the header is [" + indication + "]. Ignore it.");
	...
	private static ExecutionEnvironment detectSide() {
		...
		if (specifiedSide != null) {
			...
			default:
				Logging.LAUNCH
						.warning("Invalid value [" + specifiedSide + "] for parameter " + PROP_SIDE + ", ignoring.");
				break;
			}
		}
	...
	private static List<URLFilter> createFilters(YggdrasilConfiguration config) {
		...
		if (Boolean.TRUE.equals(config.getMeta().get("feature.legacy_skin_api"))) {
			Logging.CONFIG
					.info("Disabled local redirect for legacy skin API, as the remote Yggdrasil server supports it");
		} else {
	...
	public static ConstantURLTransformUnit unit;
	...
	private static ClassTransformer createTransformer(YggdrasilConfiguration config) {
		...
		transformer.units.add(unit = new ConstantURLTransformUnit(urlProcessor));
